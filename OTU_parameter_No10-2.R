...
colset1 <- c(brewer.pal(12,"Set3"))
colset2 <- brewer.pal(12,"Paired")
colset3 <- c(brewer.pal(12,"Paired"),brewer.pal(9,"Pastel1"))
colset4 <- c(brewer.pal(12,"Paired"),brewer.pal(9,"Pastel1"),brewer.pal(9,"Set1"))
...
# ps0 <- import_biom("./data_all/No_11_zhuyuxi/20210412_raw data_Zhu Yu-Xi/insect microbiota/01.OTU_Taxa/original/otu_taxa_table.biom")
# ps0
# sample_names(ps0)
# map = read.csv("./data_all/No_11_zhuyuxi//map_insect.csv")
# row.names(map) = map$ID
# sample_data(ps0) = map
# saveRDS(ps0,"ps_insect.rds")
# 
# ps0 <- import_biom("./data_all/No_11_zhuyuxi/20210412_raw data_Zhu Yu-Xi/insect_leaf_root_soil microbiota//01.OTU_Taxa/original/otu_taxa_table.biom")
# ps0
# sample_names(ps0)
# map = read.csv("./data_all/No_11_zhuyuxi//map_soil.csv")
# row.names(map) = map$ID
# sample_data(ps0) = map
# tax = vegan_tax(ps0)
# colnames(tax) = c("Kingdom","Phylum","Class","Order","Family","Genus","Species")
# tax_table(ps0) = as.matrix(tax)

# saveRDS(ps0,"./data_all/No_11_zhuyuxi/ps_soil.rds")
#

# tax = vegan_tax(ps0)
# colnames(tax) = c("Kingdom","Phylum","Class","Order","Family","Genus","Species")
# tax_table(ps0) = as.matrix(tax)
# saveRDS(ps0,"./data_all/No_11_zhuyuxi/ps_insect.rds")
...
map$Spieces[map$Spieces == "L.chinensis"] = "L_chinensis"
map$Spieces[map$Spieces == "L.huidobrensis"] = "L_huidobrensis"
map$Spieces[map$Spieces == "L. trifolii"] = "L_trifolii"
map$Spieces[map$Spieces == "L.sativae"] = "L_sativae"
map$Group = map$Spieces
...
if (AM) {
  ...
  ps0 = readRDS("./data_all/No_9_jiangshangtao/AM//ps.rds")
  res1path <- paste(result_path,"/OTU_base_diversity_AM",sep = "")
  dir.create(res1path)
}
...
if (Fun) {
  ...
  if (length(TFdir.f) != 0) {
    print("ITS")
    res1path <- paste(result_path,"/OTU_base_diversity_ITS",sep = "")
  }
}
...
ps <- ps%>%
  subset_taxa(
    ...
    !row.names(tax_table(ps))%in%c("OTU9")
  )
ps
