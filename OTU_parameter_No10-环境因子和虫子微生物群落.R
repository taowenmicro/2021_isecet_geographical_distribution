...
ps = ps0
head(map)
ps = filter_taxa(ps0, function(x) sum(x ) > 500, TRUE)



# ps_sub <- subset_samples(ps0,SampleType %in% c("OF"));ps_sub 
otu = t(vegan_otu(ps0))
map = sample_data(ps0)
head(map)
...
env = map
env$Group = NULL
env$Population = as.factor(env$Population) %>% as.numeric()
env$Abbreviation = as.factor(env$Abbreviation)%>% as.numeric()
env$sex = as.factor(env$sex)%>% as.numeric()
env$host.plant = as.factor(env$host.plant)%>% as.numeric()
env$Location = as.factor(env$Location)%>% as.numeric()
env$collection.date = NULL
env$Spieces = as.factor(env$Spieces)%>% as.numeric()
