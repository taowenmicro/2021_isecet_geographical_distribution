...
map$Group = gsub(", ","_",map$host.plant)
...
map$Location
#-----子集分组1 lt组#---------
ps_sub <- subset_samples(ps,Spieces %in% c("L_trifolii"));ps_sub 
ps_sub <- subset_samples(ps_sub,Location %in% c("Wuxi, Jiangsu"));ps_sub 
...
map$Location
#-----子集分组1 ls组#---------
ps_sub <- subset_samples(ps0,Spieces %in% c("L_sativae"));ps_sub 
ps_sub <- subset_samples(ps_sub,Location %in% c("Liangyungang, Jiangsu","Yancheng, Jiangsu","Xuzhou, Jiangsu"));ps_sub 
ps_sub = filter_taxa(ps_sub, function(x) sum(x ) > 1 , TRUE)
ps_sub
